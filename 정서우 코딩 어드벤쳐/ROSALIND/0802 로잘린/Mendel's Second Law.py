...
# so we can simply calculate each of the individual probabilities separately and then multiply them.

# More generally, 
# random variables X and Y are independent if whenever A and B are respective events for X and Y, 
...
# the number of pips showing on one die should not affect the number showing on the other die. If we want to find the probability that X+Y is odd, 
# we don't need to draw a tree diagram and consider all possibilities. We simply first note that for X+Y to be odd, 
# either X is even and Y is odd, or X is odd and Y is even. In terms of probability, 
# Pr(X+Y is odd)=Pr(X is even and Y is odd)+Pr(X is odd and Y is even). 
# Using independence, this becomes [Pr(X is even)×Pr(Y is odd)]+[Pr(X is odd)×Pr(Y is even)], or (1/2)**2 + (1/2)**2=1/2. 
...
# (Do not count the Aa Bb mates at each level.) 
...
for i in range(N,n+1):
    ...
    AaBb_possibility += prob

print(round(AaBb_possibility,3))
